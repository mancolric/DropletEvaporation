function [b, J] = FluxSourceRestriction(model, t, usol, fes, ComputeJ)
    ...
    %Allocate rhs vector:
    nVars       = length(usol);
    b           = zeros((model.nDiff+model.nAlg)*fes.nDof, 1);
...
%     return
    ...
    if ComputeJ
        J       = sparse(   cat(1, iv(:), iv_ee(:), iv_ew(:), iv_we(:), iv_ww(:), iv_11(:), iv_22(:), iv_g(:)), ...
                            cat(1, jv(:), jv_ee(:), jv_ew(:), jv_we(:), jv_ww(:), jv_11(:), jv_22(:), jv_g(:)), ...
                            cat(1, sv(:), sv_ee(:), sv_ew(:), sv_we(:), sv_ww(:), sv_11(:), sv_22(:), sv_g(:)) );
    else
        J       = zeros(0,0);
    end
