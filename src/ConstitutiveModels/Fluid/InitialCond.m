...
function [l, varargout] = Residue(ctes, bool)
    ...
    B_oq        =  ((obj.diff_h / obj.nu)+(obj.cpFuel*(obj.T_inf - ctes(2))))/obj.hfg;
    if nargout==1
        l(1)            = ctes(1) - 4*pi*obj.kg*obj.rs*log(1+B_oq)/obj.cpFuel;
        l(2)            = obj.hfg*(obj.nu*B_oq-1)/(obj.cpO2*(1+obj.nu)) + ctes(2) - ctes(3);
