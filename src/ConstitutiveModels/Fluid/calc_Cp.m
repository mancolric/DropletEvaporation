function Cp = calc_Cp(T_m, y, model)
...
if model.bool_liq==1 % Liquid
    Yi_eval = zeros(size(model.comp_inerts,2), size(T_v,2)); % Yi_eval = 0
    ...
    if model.nSpecies==1 % Monocomponent
        row_C_liq=2; id_f=model.nInerts+1;
...
elseif model.bool_liq==0 % Gas
    ...
    vector_C = MixtureRules('Cp_gas', T_v, Yi_eval, Yf_eval, model.matrix, model.gota, model.comp_inerts, model.P);
else
    error('Error calc_Cp')
end

Cp = reshape(vector_C,m,n); % Cp[1 x nElems*(2*p+1)]
