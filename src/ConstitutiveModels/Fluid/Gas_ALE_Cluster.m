...
model.Inerts_mw   = [0.0280134 0.031999 0.044009 0.01801528, 0.02801]; %[Kg/mol]
...
% end
    ...
    function Q=Qfun_fake(model, T, rhoy, x)
        Q               = repmat({zeros(114, 11)}, model.nDiff, 1);
        error('Usando Q fake')
    end
model.Q           = @Qfun;
model.Q_fake      = @Qfun_fake;

model.u1          = @(t) num2cell(ones(model.nSpecies + 2, 1));
model.uN          = @(t) num2cell(ones(model.nSpecies + 1, 1));
