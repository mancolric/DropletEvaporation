...
% end
    ...
    function Q=Qfun(model, T, rhoy, x)
        ...
        % AQUÍ ESTÁ LA CLAVE: Le decimos que la reacción está en la fila 5
        idx_rxn       = model.nInerts + 1;
        
        MatrixExpo    = model.ROrder(idx_rxn, min(1:model.nSpecies, model.nInerts+1)).';
        ...
        % 3. Aplicamos la función a todas las especies
        CROrder = cellfun(safe_pow, C, num2cell(MatrixExpo), 'UniformOutput', false);
        
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        
        A1 = model.Arr(idx_rxn, 1);
        A2 = model.Arr(idx_rxn, 2);
        A3 = model.Arr(idx_rxn, 3);
        
        % Calculamos la velocidad de reacción
        calc_omega = (A2 * T.^A3 .* exp(-A1 ./ (model.R .* T))) .* prod(cat(3, CROrder{:}), 3);
        
        % Calculamos el término fuente
        calc_Q = @(kk) model.species_mw(kk).* 1e6 .* (calc_omega .* model.DiffStoi(idx_rxn, kk));
